

/* ----- ANAL ----- */
setup.analQuotes = {
  Bottom: {
    Cowgirl: {
      Dominant: [
        "Bounce on my cock!",
        "Oh yeah, you're so good at that",
        "Such a good $mc.petname",
        "That's it, feel that cock deep inside you"
      ],
      Submissive: [
        "Please, bounce on my cock!",
        "I love it when you're on top of me like this",
        "Yes, pound me...",
        "Fuck me!"
      ],
      Switch: [
        "A-Ahhh..."
      ]
    },
    Missionary: {
      Dominant: [
        "You're so cute when you're being fucked."
      ],
      Submissive: [
        "I love that look on your face."
      ],
      Switch: [
        "Ah, so good!"
      ]
    },
    Doggy: {
      Dominant: [
        "Yeah! Take it!"
      ],
      Submissive: [
        "O-Oh, so good!",
        "Sorry if I go a lil fast all of a sudden."
      ],
      Switch: [
        "Ahh!"
      ]
    },
    Dominant: [
      "Shit, you know how to move!",
			"Ah, $mc.name.",
			"Mm, yeah, you like that, $mc.name?",
			"You feel so fucking good around me!",
			"You feel so tight, $mc.name. You were made for this!",
			"You make a good fucktoy.",
      "Yeah, that's it...",
      "Haaa... fuck yeah!"
      ],
      Submissive: [
        "Ahh, so tight, so good!",
				"Ah! Ah! Ah!",
				"Ah, $mc.name.",
				"I-It's so tight!",
				"Ah, you're so tight, $mc.name!",
				"You're doing so well! You feel amazing, $mc.name!",
				"$mc.name, $mc.name, $mc.name!",
				"Don't worry, I'll be gentle.",
        "Oh... oh...",
        "Use my ass...",
        "Oh... fuck my ass... please..."
      ],
      Switch: [
        "This feels so good!",
				"Ah! Ah! Ah!",
				"Ah, $mc.name.",
				"I-It's so tight!",
        "Oh... oh...",
        "Use my ass...",
        "Oh... fuck my ass... please...",
        "A-Ahhh..."
      ]
  },
  Top: {
    Cowgirl: {
      Childish:[
      "Whee, hehe! Bouncy, bouncy!",
      "Yeehaw, hehehe!",
      "Boingy Boingy Boingy~!"
      ],

      Dominant: [
        "Love it when I bounce on your cock?",
        "Let me enjoy this Grade A dick of yours!"
      ],
      Submissive: [
        "Fuck me hard!",
				"Ahh",
				"You feel so good inside me."
      ],
    },
    Doggystyle: {
      Childish:[
      "Arf Arf! hehehe!",
      "Gimme it Rrrruff and hard! Hehe- N-nnngh..!"
      ],
      Dominant: [
        "Yeah, give it to me!",
        "MMM.. fuck!"
      ],
      Submissive: [
        "That's it, give it to me!",
        "Yes, pound me...",
        "Fuck me!"
      ]
    },
    Missionary: {
      Childish:[
        "Hehe! Lets hold hands! H-haaah...",
        "So--ohhh...~ close we could kiss, hehe!"
      ],
      Dominant: [
        "Yeah, give it to me!"
      ],
      Submissive: [
        "I love it when you're on top of me like this.",
        "Yes, pound me...",
        "Fuck me!"
      ],
    },
    Childish:[
    "Take me to poundtown, $mc.name!",
    "R-really butt-on mashing, $mc.name! H-haah...~"
    ],
    Dominant: [
      "Yeah, like that!! Ooh!",
			"Ahh, so good...",
			"Yeah, that's it...",
			"Ah, right there!",
			"Oh yeah, get it dude!",
			"Yeah! Ram it in me!",
			"Ah... That's it...",
			"Now shut it and let me enjoy this Grade A dick of yours!",
			"Keep fucking me!",
			"Yes! Yes! Give it to me! AH!",
			"Good $mc.petname! Keep sliding that delicious cock of yours in and out of my ass!",
			"You like feeling my slutty little asshole gripping down on your cock?!",
      "Fuck, $mc.name, you've stuffed me good..."
    ],
    Submissive: [
      "Yes! That's it! Oh, yes! Yes!",
			"Make me yours!",
			"Yes! Yes! Give it to me! AH!",
			"Ahh! Just like this!",
			"Your dick is h-heaven!",
			"It's so big!",
			"Ah, right there!",
			"Oh yeah, get it dude!",
			"Ah... That's it...",
			"You're really fucking me, $mc.name!",
			"It feels so good to have you inside me!",
			"Keep fucking me!",
			"Yess... It's sooo good!",
			"You fill me up so w-well...",
      "Uwaaahh! AHHH!"
    ],
    Switch: [
      "Ah, right there!",
			"Ah... That's it...",
			"It feels so good to have you inside me!",
			"Keep fucking me!",
			"F-fuck, you're big!",
			"Keep it up, you're hitting me just right!",
			"Yess... It's sooo good!",
			"Oh! Oh fuck yeah! Yes!!!",
			"You feel so good inside me.",
			"You like being inside me, don't you?"
    ]
  }
};

/* ----- BLOWJOB ----- */
setup.blowjobQuotes = {
    Childish:[
    "Taste the Rainbow~!",
    "Sausage for you! Eggs below! Hahaha~!"
    ],
    Dominant: [
      "Oh yeah, keep sucking my cock!",
			"Yeah, suck my dick!",
			"That's right, use your $mc.body.Tongue as well! You're good at sucking cock!",
			"What a good $mc.petname! You love sucking my cock, don't you?",
			"Use your tongue as well! Yeah, like that!",
			"Yes! Keep sucking my cock! Just like that!",
			"Oh yes! Wrap those lips of yours around my cock! Keep going!",
			"Keep sucking my cock! Yes! Just like that!",
			"Fuck, that feels incredible.",
			"That's really good, $mc.name... you're a natural at this.",
      "You're so good at sucking dick.",
      "A-Ah...",
      "Yesss, suck it",
      "Mmm, fuck yeah..."
    ],
    Submissive: [
      "You're good at sucking cock!",
			"Ahh Ahh",
			"Ah! Please, more!",
			"Your tongue is so soft...",
			"Fuck, that feels incredible.",
			"OOOH, your mouth feels so good $mc.name!",
      "You're doing so great."
    ],
    Switch: [
      "You're good at sucking cock!",
			"Fuck, that feels incredible.",
      "Ah...",
      "Ahh Ahh",
      "Oh yeah...",
			"Your mouth is so warm...",
      "Ah, that's nice",
      "Yeah, put that mouth to good use..."
    ]
};


setup.EnterMouthQuotes = [
 "<i>MMPH MM MMMM </i>"
];

setup.TakeCockBetweenLipsQuotes = {
    Dominant:["Yeah, take it!"],
    Submissive: ["Ohh, wow, it's warm."],
    Switch: ["Ah, that's nice"]
};



/* ----- BOOBJOB ----- */
setup.BoobjobQuotes = {
  Bottom: {
    Dominant: [ "BOTTOM DOM BOOBJOB" ],
    Submissive: [ "BOTTOM SUB BOOBJOB" ],
    Switch: [ "BOTTOM SWITCH BOOBJOB" ]
  },
  Top: {
    Dominant: [ "TOP DOM BOOBJOB" ],
    Submissive: [ "TOP SUB BOOBJOB" ],
    Switch: [ "TOP SWITCH BOOBJOB" ]
  }
};

/* ----- CARESS ----- */
setup.caressQuotes = {
  Bottom: {
    Skin:{
      Dominant: ["You like me feeling you up?"],
      Submissive: ["Your skin is so smooth."],
      Switch: ["Your skin is so smooth."]
    },
    Scales:{
      Dominant: ["You like me feeling you up?"],
      Submissive: ["Your scales have such a smooth pattern."],
      Switch: ["Your scales have such a smooth pattern."]
    },
    Feathers:{
      Dominant: ["You like me feeling you up?"],
      Submissive: ["Your feathers feel light and gentle."],
      Switch: ["Your feathers feel light and gentle."]
    },
    Fur:{
      Dominant: ["You like me feeling you up?"],
      Submissive: ["Your fur is so warm… it feels comforting."],
      Switch: ["Your fur is so fluffy!"]
    }
  },
  Top: {
    Childish:[
      "That tickles, hehe!",
      "Aah~ That feels nice, $mc.name~",
      "Purrrr....~"
    ],
    Skin: {
      Dominant: ["Yeah, feel me up!"],
      Submissive: ["Mmm, that feels nice."],
      Switch: ["That feels nice."]
    },
    Scale: {
      Dominant: ["Yeah, that feels nice."],
      Submissive: ["Your hands feel good on my scales…"],
      Switch: ["That feels nice."]
    },
    Feather: {
      Dominant: ["Yeah, feel my feathers."],
      Submissive: ["That feels nice. I like your touch."],
      Switch: ["That feels nice."]
    },
    Fur: {
      Dominant: ["Like my fluffy fur?"],
      Submissive: ["That feels good. It's relaxing."],
      Switch: ["That feels nice."]
    }
  }
};
/* ----- CUDDLE ----- */
setup.cuddleQuotes = {
  Bottom: {

    Childish:[
    "Happy snugs after sticky fun! Yayy~!",
    "That was fun, $mc.name! Cum play again sometime, Hehe!"
    ],
    Dominant: [
      "Hope you don't mind some cuddling after all that.",
			"You can use my tits as a pillow if you want.",
      "Come here. I've got you.",
      "Relax. Stay close like this.",
      "Come here. I want you pressed against me.",
      "Relax. Let me hold you while your body's still warm.",
      "Stay close. I like feeling you breathe against me.",
      "Come here. You did good—just relax against me.",
      "Don't move yet. I like you like this, all spent and warm.",
      "Stay close. Let your breathing slow down."
    ],
    Submissive: [
      "This is nice.",
      "This feels safe… I like being this close.",
      "I love being held like this… it makes me feel small and safe.",
      "Your body feels so warm wrapped around mine.",
      "I'm so tired… holding you feels perfect right now.",
      "My body's still trembling a little. Please stay."
    ],
    Neutral: [
      "It's nice just holding each other for a while.",
      "It feels good just lying together like this.",
      "I don't need anything else right now.",
      "That took a lot out of me… this feels nice.",
      "I just want to lie here and catch my breath with you."
    ]
  },
  Top: {
    Childish:[
    "Happy snugs after sticky fun! Yayy~!",
    "That was fun, $mc.name! Cum play again sometime, Hehe!"
    ],
    Dominant: [
      "Your body is so soft and warm.",
			"You can use my tits as a pillow if you want.",
			"You like the feeling of me being this close, don't you?",
      "You like being held, don't you?",
      "Mm… you're nice and warm against me.",
      "I like the way you cling when I pull you closer.",
      "Don't move. I want you right here.",
      "Mm… you look completely worn out.",
      "Good. Let yourself rest against me.",
      "I can feel your heartbeat slowing down."
    ],
    Submissive: [
      "Your arms feel really comforting...",
      "Your arms feel so good around me… please don't let go.",
      "I could melt like this if you keep holding me.",
      "I'm exhausted… but being held like this feels amazing.",
      "My legs barely want to move anymore."
    ],
    Neutral: [
      "This is comfortable. I could stay like this.",
      "Your arms feel so good around me… please don't let go.",
      "I could melt like this if you keep holding me.",
      "I'm really tired… in a good way.",
      "Let's stay like this for a bit."
    ]
  }
};

/* ----- CUNNILINGUS ----- */
setup.cunnilingusQuotes = {
  Bottom: {
    Dominant: [
      "That's right, keep eating me out!",
			"Good _n.petname! Keep that $mc.body.Tongue of yours busy!",
			"What a good $mc.petname! You love the taste of my pussy, don't you?!",
			"Oh yes! Get that $mc.body.Tongue in deep!",
			"Good _n.petname! Get that $mc.body.Tongue of yours in deep!",
			"Keep going! My pussy loves your $mc.body.Tongue!",
			"Don't stop! Deeper! Eat me out! Yes, yes, yes!",
			"Hope you like the taste.",
      "Damn, you feel great down there."
    ],
    Submissive: [
      "Please keep going! My pussy loves your $mc.body.Tongue!",
			"Yes! I love your $mc.body.Tongue! Please don't stop!",
			"Don't stop! Deeper! Eat me out! Yes, yes, yes!",
			"Oh yes! Taste my pussy! I love your $mc.body.Tongue! Get it deeper!"
    ],
    Switch: [
      "Don't stop! Deeper! Eat me out! Yes, yes, yes!",
			"Mmmm"
    ]
  },
  Top: {
    Dominant: [
      "Mmmf",
			"Mmmph",
			"Mmf"
    ],
    Submissive: [
      "Mmmf",
			"Mmmph",
			"Mmf"
    ],
    Neutral: [
      "Mmmf",
			"Mmmph",
			"Mmf"
    ]
  }
};

/* ----- EMBRACE ----- */
setup.embraceQuotes = {
  Bottom: {
    Childish:[
      "Snuggle Stuffins! Yayy..~! H-haah ha..~",
      "This is my.. A-aah.. fave kinda thing!"
    ],
    Dominant: [
      "Heh, come here!"
    ],
    Submissive: [
      "This is nice.",
      "Gimme a hug!"
    ],
    Neutral: [
      "I like hugs!"
    ]
  },
  Top: {
    Childish:[
      "Snuggle Stuffins! Yayy..~! H-haah ha..~",
      "This is my.. A-aah.. fave kinda thing!"
    ],
    Dominant: [
      "Heh, feeling affectionate?"
    ],
    Submissive: [
      "Aw, wanted a hug?"
    ],
    Neutral: [ 
      "Aw..."
    ]
  }
};

/* ----- FACE ON CHEST ----- */
setup.faceonchestQuotes = {
    Large: [
      "Can you feel my heart beat? Or are my breasts too big?",
			"I can tell you like my breasts.",
			"Do you like my breasts?"
      ],
    Busty: [
      "Can you feel my heart beat? Or are my breasts too big?",
			"I can tell you like my breasts.",
			"Do you like my breasts?"
      ],
    Dominant: [
      "Bet you like that, huh?",
      "Ah!",
      "Hah"
    ],
    Submissive: [
      "Aww.",
			"You're making my chest feel good."
    ],
    Switch: [
      "Is my chest comfy?"
    ]
};

/* ----- FEEL STOMACH ----- */
setup.feelstomachQuotes = {
  Bottom: {
    
    Skinny: [
      "Your belly's light, almost hollow.",
			"Barely anything to rub.",
			"I love how delicate it feels."
    ],
    Lean: [
      "Light frame. My hand fits perfectly.",
			"It's all smooth and narrow.",
      "Feels so smooth.",
			"You're so tight and lean here.",
      "Firm, love how it feels against my hand.",
			"You've got clean definition."
    ],
    Petite: [
      "It feels small and gentle under my hand.",
			"Your tiny belly is adorable to touch."
    ],
    Wiry: [
      "Your lines show even when relaxed.",
			"It's like it was carved from a statue."
    ],
    Ripped: [
      "Your midline shows clearly.",
			"I can feel the lines of your muscles clearly.",
      "Nice and firm.",
			"You've got a healthy body."
    ],
    Thin: [
      "So flat and smooth.",
			"I love how flat it feels here."
    ],
    Toned: [
      "Your belly feels very firm.",
			"Feels good.",
			"Your belly's got a nice feel to it."
    ],
    Lithe: [
      "Nice muscles under here.",
			"Flexible and firm."
    ],
    Muscular: [
      "Nice and solid frame.",
			"Such a sturdy build."
    ],
    Plump: [
      "Love how soft and warm it is here, like a pillow.",
			"Nice cushion under my hand.",
      "Look at it jiggle.",
			"I love how your belly feels."
    ],
    Thick: [
      "Nice and smooth.",
			"I like the way you feel under my hand.",
      "There's a lot to love here!",
			"So soft..."
    ],
    Brawny: [
      "Nice and smooth muscles.",
			"Really nice definition."
    ],
    Ample: [
			"You've got a good tone.",
      "These muscles are so thick!",
			"You've got real heft here… it feels good to touch."
    ],
    Round: [
      "Nice and round.",
			"I love how it jiggles!"
    ],
    Portly: [
      "Thick and plush.",
			"I could use it like a pillow."
    ],
    Stocky: [
      "You've got a nice solid core.",
			"So much strength behind this."
    ],
    Jacked: [
      "Wow, look at these abs!",
			"Such sexy abs!",
			"You can grind meat with these abs!",
      "It's as solid as a rock!",
			"Nothing but pure muscle, amazing!"
    ]
  },
  Top: {
    Childish:[
      "That tickles, hehe!",
      "Aah~ That feels nice, $mc.name~",
      "Purrrr....~"
    ],
    Fur: [
      "You like my fur, huh?"
    ],
    Scales: [
      "You like my scales, huh?"
    ],
    Feathers: [
      "You like my feathers, huh?"
    ],
    Neutral: [
      "Mmm, that feels good."
    ]
  }
};

/* ----- FINGER ASS ----- */
setup.FingerAssQuotes = {
  Bottom: {
    Dominant: [
      "Yeah, lets get it nice and stretched."
    ],
    Submissive: [
      "It's so warm in there."
    ],
    Switch: [
      "It's warm."
    ]
  },
  Top: {
    Childish: [
      "Press the Butt-on for funny noises! Baaa-aah...",
      "Press the Butt-on for funny noises! Mooo--ooh~",
      "Press the Butt-on for funny noises! Quaaah..ck~",
      "Press the Butt-on for funny noises! Arrf~",
      "Press the Butt-on for funny noises! R-ribbit...~",
      "Press the Butt-on for funny noises! Awooog-aaah~!",
      "Press the Butt-on for funny noises! Me-aaaooow~",
      "Press the Butt-on for funny noises! Caw Caaaahw Caw..~!"
    ],
    Dominant: [
      "Oh f-fuck!"
    ],
    Submissive: [
      "Oh f-fuck!"
    ],
    Switch: [
      "Oh f-fuck!",
      "H-Haaa... Ahhh!"
    ]
  }
};

/* ----- FINGER VAGINA ----- */
setup.fingerpussyQuotes = {
  Bottom: {
    Dominant: [
      "Damn, you're wet!",
      "Ohh, you're so wet.",
      "Look at you... so wet.",
      "Mmm, already wet."
    ],
    Submissive: [
      "It's so warm in there.",
			"Wow, its so wet down there."
    ],
    Switch: [
      "It's warm in there.",
      "Fuck, your pussy is so wet.",
      "Your pussy's so wet."
    ]
  },
  Top: {
    Dominant: [
      "Oh f-fuck!",
			"That feels nice.",
			"Yes... just like that!",
			"That's the spot, don't stop!"
    ],
    Submissive: [
      "Oh f-fuck!",
      "You're making my pussy feel so good."
    ],
    Switch: [
      "Oh f-fuck!",
      "You're making my pussy feel so good."
    ]
  }
};

/* ----- FOOTJOB ----- */
setup.footjobQuotes = {
  Bottom: {
    Childish:["Wow! No hands needed! Nice, $mc.name!"],
    Dominant: ["Good, keep doing that."],
    Submissive: [
      "Your feet are so soft.",
			"That's great, $mc.name! Keep it up!",
			"Your feet feel nice."
    ],
    Switch: [
      "Your feet are so soft.",
			"That's great, $mc.name! Keep it up!",
			"Your feet feel so nice.",
			"You really know how to handle your feet."
    ]
  },
  Top: {
    Childish:[
    "You're gettin the Beeeeeeans~!",
    "Look! No hands!"
    ],
    Dominant: [
      "I can feel your pulse through the soles of my feet, $mc.name."
    ],
    Submissive: [
      "I really like how you feel against my toes.",
			"I can feel your cock pulse, $mc.name.",
			"I wonder how it'll feel to have your cum squirt into my soft, bare feet?"
    ],
    Switch: [
      "I can feel your pulse through the soles of my feet, $mc.name."
    ]
  }
};

/* ----- FROT ----- */
setup.frotQuotes = {
  Bottom: {
    Childish:[
    "Kissy kissy PPs! Hahaha~!",
    "Wub wub~ Nnh...~"
    ],
    Dominant: [
      "Ahh yeah.",
			"Fuck yeah, this is good!",
      "Y-Your <<=$mc.genitals.toLowerCase()>> feels so good, $mc.name..."
    ],
    Submissive: [
      "Hope I'm not moving too fast.",
			"Y-you feel so good."
    ],
    Switch: ["Ah!"]
  },
  Top: {
    Childish:[
    "Kissy kissy PPs! Hahaha~!",
    "Wub wub~ Nnh...~"
    ],
    Dominant: [
      "Mmm, this feels so good.",
      "Y-You feel so good $mc.name!"
    ],
    Submissive: [
      "O-Oh wow...",
			"P-Please keep going."
    ],
    Switch: [
      "Ah.",
      "Augh...!"
    ]
  }
};

setup.TribQuotes = [
  "You like the feel of my pussy rubbing over yours?",
  "That's it, push your pussy back against mine!",
  "Our juices are mingling together."

];

/* ----- GROPE BREAST ----- */
setup.gropebreastQuotes = {
  Bottom: {
    Dominant: {
      Flat: {
        Kid: [
          "I guess I shouldn't be expecting anything yet.",
          "You're so hot. I love small breasts."
        ],
        Teen: ["It's kinda cute."],
        Adult: ["Don't be ashamed about being flat, it's really cute."]
      },
      Small: {
        Kid: ["Nothing quite like developing breasts. So lewd and cute."],
        Teen: ["Nothing quite like developing breasts. So lewd and cute."]
      },
      Pert: {
        Kid: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Teen: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Adult: [
          "Nice tits.",
          "Such perfect perky breasts.",
          "Such nice breasts."
        ]
      },
      Modest: {
        Kid: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Teen: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Adult: [
          "Nice tits.",
          "Such perfect perky breasts.",
          "Such nice breasts."
        ]
      },
      Perky: {
        Kid: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Teen: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Adult: [
          "Nice tits.",
          "Such perfect perky breasts.",
          "Such nice breasts."
        ]
      },
      Full: {
        Kid: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Teen: [
          "Nice tits.",
          "Such nice breasts."
        ],
        Adult: [
          "Nice tits.",
          "Such perfect perky breasts.",
          "Such nice breasts."
        ]
      },
      Large: {
        Kid: [
          "Heh, look a the size of these!",
				  "Damn, these are huge!"
        ],
        Teen: [
          "Heh, look a the size of these!",
				  "Damn, these are huge!"
        ],
        Adult: [
          "Heh, look a the size of these!",
				  "Damn, these are huge!",
          "I love your big breasts.",
          "Your huge breasts are so hot.",
          "Love your big breasts."
        ]
      },
      Ample:[
        "Shit, these are big!",
				"How do you carry these things?!"
      ],
      Massive:[
        "Shit, these are big!",
				"How do you carry these things?!"
      ],
      Enormous:{
        Kid: ["How the hell is someone your age able to carry these things?!"],
        Teen: ["How the hell is someone your age able to carry these things?!"],
        Adult: [ "Shit, these are big!",
				"How do you carry these things?!"
        ]
      }
    },
    Submissive: {
      Flat: {
        Kid: ["They're soft."],
        Teen: ["I'm sure they're develop."],
        Adult: ["I can work with this."]
      },
      Small: {
        Kid:["Cute and developing."],
        Teen: ["It's ok being a late bloomer."],
        Adult: ["Cute."]
      },
      Average: {
        Kid: ["Nice tits."],
        Teen: ["Nice tits."],
        Adult: ["Nice tits."]
      },
      Large: [
        "Your breasts are so soft.",
				"Wow, they are pretty big."
      ],
      Ample: ["Whoa..."],
      Massive: ["These sure are big!"],
      Enormous: ["These sure are big!"]
    },
    Switch: {
      Flat: {
        Kid: ["They're soft."],
        Teen: ["Your breasts are so soft."],
        Adult: ["Your breasts are so soft."]
      },
      Small: {
        Kid: ["They're soft."],
        Teen: ["Your breasts are so soft."],
        Adult: ["Your breasts are so soft."]
      },
      Average: {
        Kid: ["They're soft."],
        Teen: ["Your breasts are so soft."],
        Adult: ["Your breasts are so soft."]
      },
      Large: ["Wow..."],
      Ample: ["These sure are big!"],
      Massive: ["These sure are big!"],
      Enormous: ["These sure are big!"]
    }
  },
  Top: {
    Dominant: [
      "That's right, play with my <<=_n.breastsize.toLowerCase()>> breasts!",
      "That's right, play with my tits!"
    ],
    Submissive: [
      "You can play with them if you like."
    ],
    Switch: {
      Flat: ["Like my breasts?"],
      Tiny: ["Like my breasts?"],
      Small: ["Like my breasts?"],
      Pert: ["Like my breasts?"],
      Modest: ["Like my breasts?"],
      Perky: ["Like my breasts?"],
      Full: ["Like my breasts?"],
      Large: ["Like my breasts?"],
      Ample: ["Like my breasts?"],
      Massive: ["Like my breasts?"],
      Enormous: ["Like my breasts?"]
    }
  }
};


/* ----- GROPE BUTT ----- */
setup.gropebuttQuotes = {
  Bottom: {  
    Flat: {  
      Dominant: [     
      "Yeah, you like this?"
      ],
      Submissive: [
      "I love how your ass feels."
      ]  
    },
    Tiny: {  
      Dominant: [     
      "Let me just enjoy this tiny butt."
      ],
      Submissive: [
      "I love how your ass feels."
      ] 
    },
    Small: {  
      Dominant: [   
      "I love how your ass feels."   
      ],
      Submissive: [
      "I love how your ass feels."
      ]  
    },
    Round: {  
      Dominant: [
      "I love how round your ass is.",
      "I love how your ass feels.",
			"You like me grabbing your ass like this?",
			"I like playing with your ass."
      ],
      Submissive: [
      "I love how round your ass is.",
      "I love how your ass feels."
      ]   
  },
    Large: {  
      Dominant: [
      "I love how your ass feels.",
			"You like me grabbing your ass like this?",
			"I like playing with your ass."
      ],
      Submissive: [
      "I love how your ass feels."
      ]
    },
    Huge: {  
      Dominant: [
      "I love how your ass feels.",
			"You like me grabbing your ass like this?",
			"I like playing with your ass."
      ],
      Submissive: [
      "I love how your ass feels."
      ]
    },
    Neutral: [
      "I love how your ass feels."
      ]  
  },  
  Top: {
    Childish:[
    "Aah! Hehe!",
    "Fun to squish huh? Hahaha~!",
    "Doubles as a good pillow!"
    ],
    Flat: {  
      Dominant: [
      "Your hands are so warm..."
      ],
      Submissive: [
      "Your hands are so warm..."
      ],
      Neutral: [
      "Your hands are so warm..."
      ]   
    },
  Tiny: {  
      Dominant: [
      "Your hands are so warm..."
      ],
      Submissive: [
      "Your hands are so warm..."
      ],
      Neutral: [
      "Your hands are so warm..."
      ]  
    },
  Small: {  
      Dominant: [
      "Your hands are so warm..."
      ],
      Submissive: [
      "Your hands are so warm..."
      ],
      Neutral: [
      "Your hands are so warm..."
      ]   
    },
  Round: {  
      Dominant: [
      "Your hands are so warm..."
      ],
      Submissive: [
      "Your hands are so warm..."
      ],
      Neutral: [
      "Your hands are so warm..."
      ]   
    },
  Large: {  
      Dominant: [
      "Your hands are so warm..."
      ],
      Submissive: [
      "Your hands are so warm..."
      ],
      Neutral: [
      "Your hands are so warm..."     
      ]  
    },
  Huge: {  
      Dominant: [ "Your hands are so warm..." ],
      Submissive: [ "Your hands are so warm..."],
      Neutral: [ "Your hands are so warm..."]    
    },
  }
};

/* ----- HANDJOB ----- */
setup.handjobQuotes = {
  Bottom: {
    Dominant: [
      "Mmm, your hand feels good around my cock.",
      "Your hands are soft.",
      "Yeah, jerk me off!"
    ],
    Submissive: [
      "Mmm, that feels nice.",
      "Ohh."
    ],
    Switch: [
      "Mmm, that feels nice.",
      "H-Hahhh... Fuck!",
      "A-Ahhh, $mc.name..."
    ]
  },
  Top: {
    Childish:[
      "Lemme give you a hand buddy!"
    ],
    Dominant: {
      Tiny: {
        Kid: [
          "Perfect size for someone your age.",
          "So cute"
        ],
        Adult: [
          "So cute and tiny.",
          "Wow, it's small.",
          "What a cute little dick",
          "Such a tiny dick."
        ]
      },
      Small: ["So small, yet so precious."],
      Average: [
        "Your cock is so pretty.",
        "Wow, this is a really nice dick, $mc.name!"
      ],
      Large: [
        "Damn, that's pretty big.",
        "That's a nice cock.",
        "Nice and big.",
        "Such a big cock."
      ],
      Huge: [
        "Fuck, you're hung!",
        "Fuck, your cock is huge.",
        "Holy shit, that's so massive.",
        "Holy fuck, that's huge."
      ]
    },
    Submissive: {
      Tiny: {
        Kid: ["Perfect size for someone your age."],
        Adult: ["So cute and tiny."]
      },
      Small: ["Don't feel bad about the size, I like it."],
      Average: [
        "Perfect size."
      ],
      Large: [
        "Wow, it's pretty big!",
        "You should be proud to have a cock this big!",
        "Yeah, let me jerk it."
      ],
      Huge: ["It's quite an intimidating size."]
    },
    Switch: [
     "Yeah, such a nice cock to jerk off!"
    ]
  }
};

setup.GrabCockQuotes = {
  Bottom: {
    Dominant: [
      "Mmm, your hand feels good around my cock.",
      "Your hands are soft."
    ],
    Submissive: [
      "Mmm, that feels nice.",
      "Ohh."
    ],
    Switch: [
      "Mmm, that feels nice."
    ]
  },
  Top: {
    Childish:[
      "Lemme give you a *hand* buddy!"
    ],
    Dominant: {
      Tiny: {
        Kid: [
          "Perfect size for someone your age.",
          "So cute"
        ],
        Adult: [
          "So cute and tiny.",
          "Wow, it's small.",
          "What a cute little dick.",
          "Such a tiny dick."
        ]
      },
      Small: ["So small, yet so precious."],
      Average: [
        "Your cock is so pretty.",
        "Wow, this is a really nice dick, $mc.name!"
      ],
      Large: [
        "Damn, that's pretty big.",
        "That's a nice cock.",
        "Nice and big.",
        "Such a big cock."
      ],
      Huge: [
        "Fuck, you're hung!",
        "Fuck, your cock is huge.",
        "Holy shit, that's so massive.",
        "Holy fuck, that's huge."
      ]
    },
    Submissive: {
      Tiny: {
        Kid: ["Nice size for someone your age."],
        Adult: ["So cute and tiny."]
      },
      Small: ["Don't feel bad about the size, I like it."],
      Average: [
        "Perfect size."
      ],
      Large: [
        "Wow, it's pretty big!",
        "You should be proud to have a cock this big!",
        "Yeah, let me jerk it."
      ],
      Huge: ["It's quite an intimidating size."]
    },
    Switch: [
     "Wow, this is a really nice dick, $mc.name!"
    ]
  }
};

/* ----- PET EARS ----- */
setup.petearsQuotes = {
  Bottom: {
    Dominant: [
      "They feel so soft."
    ],
    Submissive: [
      "They feel so soft."
    ],
    Switch: [
      "They feel so soft."
    ]
  },
  Top: {
    Childish:[
      "That tickles, hehe!",
      "Aah~ That feels nice, $mc.name~",
      "Purrrr....~"
    ],
    Dominant: [
      "Mmm, that feels nice.",
      "Mmm..."
    ],
    Submissive: [
      "Mmm, that feels nice.",
      "Mmm..."
    ],
    Switch: [
      "Mmm, that feels nice.",
      "Mmm..."
    ]
  }
};

/* ----- RIMJOB ----- */
setup.rimjobQuotes = {
    Childish:[
    "Pucker up 'n kiss my pucker! Hehehe!",
    "A-aah.. hope you're enjoying th-aah.. doughnut hole! Hehe!",
    "Taste the Rain-butt!"
    ],
    Dominant: [
      "Fuck, eat my ass!",
			"Good _n.petname! Keep that $mc.body.Tongue of yours busy in my ass!",
			"What a good _n.petname! You love the taste of my ass, don't you?!",
			"Don't stop! Deeper! Get your $mc.body.Tongue into my ass! Yes, yes, yes!",
			"Fuck yeah!",
			"Get in there more!"
    ],
    Submissive: [
      "Yes! I love your $mc.body.Tongue! Please don't stop!",
			"Oh yes! Taste my ass! I love your $mc.body.Tongue! Get it deeper!"
    ],
    Switch: [
      "O-oh fuck!",
			"Ah, fuck.",
      "Mmm, your tongue feels so good against my ass!"
    ]
};

/* ----- SUCK FOOT ----- */
setup.suckfootQuotes = {
  Bottom: {
    Childish:[
    "H-hey, that tickles!!! Stoppit..!",
    "A-aah! N-n-not too much of that..! I might kick you on accident..!"
    ],
    Dominant: [
      "Good _n.petname, keep sucking on my _n.body.Toes!",
			"That's right, keep swirling your $mc.body.tongue around my _n.body.Toes!",
			"What a good _n.petname! You love sucking on my _n.body.Toes, don't you?",
			"That's right, keep sucking on my _n.body.Toes!",
			"Keep sucking on my _n.body.Toes, just like that!"
    ],
    Submissive: [
      "I love having my _n.body.Toes sucked! Keep going!",
			"Ah, that feels funny."
    ],
    Switch: [
      "Mmm, that feels nice."
    ]
  },
  Top: {
    Dominant: [
      "Mmmf",
			"Mmmph",
			"Mmf"
    ],
    Submissive: [
      "Mmmf",
			"Mmmph",
			"Mmf"
    ],
    Switch: [ 
      "Mmmf",
			"Mmmph",
			"Mmf"
    ]
  }
};

/* ----- ORGASM ----- */
setup.orgasmQuotes = {
  Bottom: {
    "in ass": {
      Dominant: [
        "I'm cumming!",
        "I'll fill you up nicely",
			  "I'm fucking cumming!",
			  "Here it comes!"
      ],
      Submissive: [
        "I'm cumming!",
			  "I'm... gonna cum!",
        "A-agh!! AaaHhhh!!!"
      ],
      Switch: [
        "I'm cumming!",
        "Aaaah!!"
      ]
    },
    "on dick": {
      Dominant: ["Mnnngh!!"],
      Submissive: ["Mnnngh!!"],
      Switch: ["Ooooh!!"]
    },
    "on hand": {
      Dominant: ["Mnnngh!!"],
      Submissive: ["Mnnngh!!"],
      Switch: ["Mnnngh!!"],
      "Punk": ["You idiot, look what you made me do..."]
    },
    "in mouth":{
      Dominant: ["Mnnngh!!"],
      Submissive: ["Mnnngh!!"],
      Switch: ["Mnnngh!!"],
    },
     /* DEFAULT FALLBACK */
    default: {
      Dominant: ["Ahh. That was a good fuck."],
      Submissive: ["Phew, that felt good."],
      Switch: ["Mmm, that was nice."]
    }
  },
  Top: {
    "in ass": {
      Childish: [
      "Relax n let a load off $mc.name~ Mmm...~",
      "Hee hee, Yayy! I feel extra warm inside now!",
      "Yayyy! You crossed the finish line! Winner~!"
      ],
      Dominant: [
        "Fuck that was alot.",
        "Fuck, you really filled me up, _n.name...",
        "Mmf! That's the stuff!"
      ],
      Submissive: [
        "You're filling me up!"
      ],
      Switch: [
        "M-my ass... you filled me up so much!"
      ]
    },
    "in pussy":{
        Dominant:["Fuck, fill me up!"],
        Submissive:["OOOHHHH!"],
        Switch:["Oh Yeah!"]
    },
    "on penis": {
      Childish:[
      "Have some Cumfetti! Happy Birthday! Hehehe!",
      "You need more colors! Here you go hehehe~!"
      ],
      Dominant: ["Fuck yeah!..."],
      Submissive: ["A-Ahhh...!"],
      Switch: ["A-Ahhh...!"]
    },
    "on face": {
      Dominant: ["Really made a mess of your face."],
      Submissive: ["Really made a mess of your face."],
      Switch: ["Really made a mess of your face."]
    },
    "on self": {
      Dominant: ["Now you're nice and messy."],
      Submissive: ["Now you're nice and messy."],
      Switch: ["Now you're nice and messy."]
    },
     /* DEFAULT FALLBACK */
    default: {
      Dominant: ["Damn, enjoyed yourself?"],
      Submissive: [
        "Wow, that was nice.",
        "That was awesome."
      ],
      Switch: [
        "Phew...",
        "That was awesome."
      ]
    }
  }
};

/* ----- ORGASM CLOSE ----- */
setup.orgasmcloseQuotes = {
  OT:{
    /*The Top is Cumming */
    Bottom: {
    /* You are the bottom */
      "Take Cock In Ass": {  
        Dominant: [
        "I'm gonna cum in your ass!",
        "I'm gonna pump you full of my cum!",
        "Here it comes!"
        ],
        Submissive: [
          "I'm... gonna cum!", 
          "A-agh!! AaaHhhh!!!",
          "I can't hold it anymore!"
        ],
        Switch: [
          "I'm gonna cum in your ass!", 
          "I'm cumming!",
          "Aaaah!!"
        ]
      },
      "Pump Shaft": {
        Childish:["I'm ready for some facepaint!"],
        Dominant: ["Don't stop jerking it, I'm so close!"],
        Submissive: ["Faster, I'm almost there!"],
        Switch: ["You're gonna jerk the cum out of me!"]
      },
      "Finger Pussy":[
        "You're gonna make me squirt!"
      ], 
      "Suck Cock": {
        Childish:[
        "Ima.. color your insides..! Nngh.!",
        "Here cums some trademark.. Oooey Goooey! Aaah..!",
        "Crossing.. the finish line! Haaah..!"
        ],
        Dominant:["I'm gonna cum in your mouth!"],
        Submissive: ["I'm gonna cum!"],
        Switch: [
          "I'm gonna cum!",
          "Hahh, Arghh!!!"
        ]
      },
      "Stroke Shaft With Feet": {
        Dominant:["I'm gonna cum on your feet!"],
        Submissive: ["I'm gonna cum on your feet!"],
        Switch: ["I'm gonna cum on your feet!"]
      },
      /* DEFAULT FALLBACK */
      default: {
        Dominant: ["Fuck, I'm gonna cum."],
        Submissive: ["A-Ah! I'm gonna cum!"],
        Switch: ["I'm gonna cum!"]
      }
    },
    Top: {
    /* You are the Top */
      Childish: [
      "Mmh..! Getting close to the finish line...!",
      "I-im about to be a fruit gusher...!"
      ],
      "Fuck Ass": {  
        Dominant: [
          "Cum in my ass!",
          "Fill my ass with your spunk!"
          ],
        Submissive: [
          "Oh god, cum in me!"
        ],
        Switch: [
          "C-cum in me!"
        ]
      },
      Handjob: {
        Dominant: [
          "Yeah, gonna squeeze it out of you!"
        ],
        Submissive: [
          "Oh, I can feel you cumming."
        ],
        Switch: [
          "Come on, cum."
        ]
      },
      "Finger Pussy": {
        Dominant: [
          "You're going to make me squirt!",
          "Gonna fucking cum.",
          "Shit, I'm gonna cum.",
          "Oh fuck, I'm gonna cum."
        ],
        Submissive: [
          "Gonna cum.",
          "I'm gonna cum.",
          "Oh fuck, I'm close.",
          "I'm so close."
        ],
        Switch: [
          "Gonna cum",
          "I'm gonna cum",
          "Oh fuck, I'm close",
          "I'm so close"
        ]
      },
      "Fuck Mouth": [
        "MMPH"
      ],
      "Lick Pussy": [
        "MMPH"
      ],
      "Fuck Feet": {
        Dominant:["Cum on my feet!"],
        Submissive: ["Please, cum on my feet!"],
        Switch: ["Cum on my feet!"]
      },
      "Fuck Feet": {
        Dominant:["Cum on my feet!"],
        Submissive: ["Please, cum on my feet!"],
        Switch: ["Cum on my feet!"]
      },
      "Fuck Pussy": [
        "Cum in my Pussy!"
      ],
      /* DEFAULT FALLBACK */
      default: {
        Dominant: ["Come on, cum!"],
        Submissive: ["I can see it, let it out."],
        Switch: ["Let it out."]
      }
    }
  },
  OB:{
    /*The Bottom is Cumming */
    Top: {
      /* You are the Top */
      "Finger Ass(Continue)": ["Keep fingering me, I'm almost there!"],
      default: {
        Dominant: ["Fuck, I'm cumming"],
        Submissive: ["I'm cumming!"],
        Switch: ["I'm cumming"]
      }
    },
    Bottom: {
    /* You are the Bottom */  
      default: {
        Dominant: ["Go ahead and let it out!"],
        Submissive: ["You can cum if you want."],
        Switch: ["Just cum."]
      }
    }
  }
};

/* ----- VAGINAL ----- */
setup.vaginalQuotes = {
  Bottom: {
    Dominant: [
      "Good _n.petname! Feel my <<=$mc.genitals.toLowerCase()>> slide deep into your little pussy!",
			"That's right, be a good _n.petname and moan for me! Feel my <<=$mc.genitals.toLowerCase()>> sliding deep into your cute little cunt!",
			"Your cute little cunt feels so good squeezing down around my <<=$mc.genitals.toLowerCase()>>!",
			"Fuck! Your pussy feels so good!",
			"Oh yes! Take my cock! Take it deep!",
			"Your pussy was made for my cock!",
			"The way you're squeezing in there, feels so damn good!",
			"Oh fuck, this feels great.",
      "That's a good $mc.petname . Take that cock",
      "Yesss, you're taking it so well",
      "Good $mc.petname. Take it...",
      "Fuck, your tight little cunt feels amazing",
      "Fuck yes, give me all of that fucking pussy",
      "Oh yeah, take my cock",
      "Fuck, that feels good",
      "You're so tight",
      "You feel so good",
      "Your pussy is so tight",
      "You feel amazing"
    ],
    Submissive: [
      "Fuck! Your pussy feels so good!",
			"Yes! Use my cock! I love your pussy!",
			"Oh yes! Use me! I love your pussy!",
			"The way you're squeezing in there, feels so damn good!",
      "Uah!",
      "Ah",
      "Ha! Ah!"
    ],
    Switch: [
			"Fuck! Your pussy feels so good!",
			"The way you're squeezing in there, feels so damn good!",
			"Your pussy is perfect!",
			"It's so warm and wet in there..."
    ]
  },
  Top: {
    Dominant: [
      "Don't stop! Harder! Fuck me! Yes, yes, yes!",
			"Fuck my pussy!",
			"It's so big!",
			"Keep fucking me!",
			"You like feeling my pussy gripping down on your cock?!",
			"Keep going! Get that <<=$mc.genitals.toLowerCase()>> in deep like a good $mc.petname!",
			"Yes! Fuck me! Fuck me harder! Don't stop!",
      "Mmm, I'm glad you like my body so much.",
      "Oh yeah, fuck me",
      "Give me that hard cock",
      "Fuck my pussy"
    ],
    Submissive: [
      "Don't stop! Harder! Fuck me! Yes, yes, yes!",
			"It's so big!",
			"I feel so full!",
      "Yes... please use me...",
      "Use my pussy...",
      "Yesss, use that pussy",
      "Use my pussy, make yourself feel good",
    ],
    Switch: [
      "Don't stop! Harder! Fuck me! Yes, yes, yes!",
			"It's so big!",
			"I feel so full!",
			"Keep fucking me!",
			"Uhnn! Fuck, fuck, fuck!",
			"Ahh!",
    ]
  }
};


setup.EnterPussyQuotes = {
  Top: {
    Dominant:["It's inside me!"],
    Submissive: ["You feel so good inside me."],
    Switch: ["It's inside me!"]
  }

};

setup.PushCockInQuotes = {
  Bottom: {
    Dominant:[
      "Fuck, your tight little cunt feels amazing.",
      "Fuck yes, give me all of that fucking pussy.",
      "Oh yeah, take my cock!"
      ],
    Submissive: [
      "Fuck, that feels good.",
      "You're so tight.",
      "You feel so good.",
      "Your pussy is so tight.",
      "You feel amazing.",
      "Your pussy is so warm."
    ],
    Switch: ["You feel so nice around my dick."]
  }
};


setup.TeaseCockWithPussyQuotes =[
  "You want to be inside me, don't you?",
  "Is this what you want?",
  "This is what you want, isn't it?"
];


setup.TeasePussyWithCockQuotes ={
  Dominant:[
    "Feel how close I am to being inside you.",
    "Feel how hard this cock is for you...",
    "Is that <<=$mc.genitals >> ready for me?"
  ],
  Submissive:[
    "I... I want to be inside you."
  ],
  Switch:[
    "I need your <<=$mc.genitals >>!"
  ]
};

/*---Top teases bottom's ass with their cock ----*/
setup.TeaseAssCockQuotes = {
  Top:[
    "Your ass is mesmerising.",
    "I'm gonna shoot my load all over you.",
    "I love fucking your asscheeks.",
    "Your ass feels amazing against my cock.",
    "I can't wait to shoot my load on you.",
    "You have such a cute ass on you."],
  Bottom:[
    "You're good at this.",
    "Don't stop. Keep going!",
    "Wanna get inside me, dont you?"
  ]
};

setup.SuckNippleQuotes =[
  "That tickles.",
	"You're making my chest feel good.",
  "Your tongue feels good against my skin.",
  "That's it. Right there."
];

setup.KissNippleQuotes =[
  "That tickles.",
	"You're making my chest feel good.",
  "Your tongue feels good against my skin.",
  "That's it. Right there.",
  "That feels so good."
];

setup.LickNippleQuotes =[
  "That tickles.",
	"You're making my chest feel good.",
  "Your tongue feels good against my skin.",
  "That's it. Right there.",
  "A-Ah!"
];

setup.TeaseNippleQuotes ={
  Top:["Yes, play with my nipples."],
  Bottom:["So perky..."]
};


setup.SuckingQuotes=[
  "<i>GLRK GLRK</i>",
	"<i>SLRP</i>"
];

setup.MuffledQuotes=[
  "Mmmf",
	"Mmmph",
	"Mmf",
  "Mmmf—rrhh… mmnnh!",
	"Hrrmph… mffh—mm!",
	"Nnff… mmphh—rrgh!",
	"Mmm—hff… nnhhmm!",
	"Rrmmf—hnn… mpphh!"
];

setup.EnterAssQuotes ={
  Top:[
    "Your cock feels so nice in my ass!",
    "Nghhh!!!"
  ],
  Bottom:[
    "You feel so nice around my dick.",
    "Your butt is so warm.",
    "Hold still, I'm going in.",
    "There, it's all in!"
  ]
};

setup.TeaseCockWithFeetQuotes =[
  "I can feel you leaking on my toes."
];

setup.TeasePussyWithFeetQuotes =[
  "Do you like the feel of my feet?"
];

setup.RubClitQuotes=[
  "Does having your clit teased feel good?",
	"Your little clit is so cute!",
  "Your clit is so much fun to play with."
];

setup.TeaseTipQuotes=[
  "Wow, you're really sensitive there."
];

setup.defaultMoan1Quotes=[
  "A-Ah!", 
  "Ahh!", 
  "Hahh, Arghh!!!",
  "Ah, so good!", 
  "A-agh!! AaaHhhh!!!",
  "Ah! Ah! Ah!",
  "Fuck me!"
];